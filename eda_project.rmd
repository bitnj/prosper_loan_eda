...
```{r echo = FALSE, message=FALSE, warning=FALSE}
ggplot(data = loan_data, aes(x = factor(Term))) +
    geom_bar()
```
...
cols_to_keep <- c('Term', 'BorrowerRate', 'EstimatedLoss', 'ProsperScore',
                  ...
                  'BankcardUtilization', 'TradesOpenedLast6Months',
                  'DebtToIncomeRatio', 'LoanOriginalAmount', 'Investors')
...
pScore_cScore <- loan_data_prosper_score[, c("ProsperScore", 
...
heatmap.2(pScore_cScore_matrix, Rowv = NA, Colv = NA, 
...
grid.arrange(ggplot(data = loan_data_prosper_score, 
                    aes(x = CreditScoreRangeLower, fill = defaulted)) +
    geom_bar() + guides(fill = FALSE) +
        scale_y_log10(),
    ggplot(data = loan_data_prosper_score,
           aes(x = DebtToIncomeRatio, fill = defaulted)) +
        geom_bar() +
        scale_x_reverse(lim = c(1, 0)) +
        scale_y_log10(), nrow = 1)
```

These barplots support the idea that defaults occur more frequently for
lower credit quality borrowers (lower credit score).  It also gives some
evidence that higher debt to income ratios play a role in defaults.  Even though
there are more defaults at lower DTI values, they are a smaller proportion of
the loans at that DTI.
...
```
```{r echo = FALSE, message = FALSE, warning = FALSE}
loss_pScoreBucket <- group_by(loan_data_prosper_score, 
                                         ProsperScore_bucket)
summarise(loss_pScoreBucket, mean(EstimatedLoss))
```
...
```
```{r echo = FALSE, message = FALSE, warning = FALSE}
default_by_pScore <- group_by(loan_data_prosper_score, 
                              ProsperScore)
summarise(default_by_pScore, default_pct = sum(defaulted,
                                               na.rm = TRUE) / n())
```
